...
def classify0(inX, dataSet, labels, k):
    ...
    distances = sqDistances**0.5 #应该为0.5
...
def handwritingClassTest():
    hwLabels = []
    trainingFileList = listdir('G:\\桌面\\jupyter notebook\\digits\\trainingDigits')           #load the training set
    ...
    for i in range(m):
        ...
        trainingMat[i,:] = img2vector('G:\\桌面\\jupyter notebook\\digits\\trainingDigits\\%s' % fileNameStr)
    testFileList = listdir('G:\\桌面\\jupyter notebook\\digits\\testDigits')        #iterate through the test set
    errorCount = 0.0
    ...
    for i in range(mTest):
        ...
        fileStr = fileNameStr.split('.')[0]     #take off .txt
        classNumStr = int(fileStr.split('_')[0])
        vectorUnderTest = img2vector('G:\\桌面\\jupyter notebook\\digits\\testDigits\\%s' % fileNameStr)
